...
def safe_tavily_search(tavily, **kwargs):
    last_error = None
    for _ in range(3):
        try:
...
def check_ticket_status(attraction: str, city: str = "", date: str = "") -> str:
    ...
    try:
        ...
        return f"错误：景点搜索服务暂时不可用，原因：{type(e).__name__}"
...
class OpenAICompatibleClient:
    ...
    def generate(self, prompt: str, system_prompt: str) -> str:
        ...
        try:
            ...
            if response is None:
                raise ValueError("LLM 返回 response=None")

            if not hasattr(response, "choices") or not response.choices:
                raise ValueError(f"LLM 返回中缺少有效 choices: {response}")

            choice0 = response.choices[0]
            if choice0 is None:
                raise ValueError(f"LLM 返回 choices[0]=None: {response}")

            message = getattr(choice0, "message", None)
            if message is None:
                raise ValueError(f"LLM 返回 message=None: {response}")

            answer = getattr(message, "content", None)

            # 有些兼容接口可能 content 为空
            if not answer:
                raise ValueError(f"LLM 返回 content 为空: {response}")
            ...
            return (
                'Thought: 我无法正常调用语言模型。\\n'
                'Action: {"type":"reply","content":"抱歉，我暂时无法调用语言模型服务，请稍后重试，或检查 API Key、base_url、账户权限与返回格式。"}'
            )
...
if __name__ == "__main__":
    API_KEY = os.getenv("MODELSCOPE_API_KEY", "ms-c73415f6-015c-4e71-9409-943832b5bb10")
    ...
    os.environ.setdefault("TAVILY_API_KEY", "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")
